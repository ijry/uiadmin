...
class Cate extends Admin
{
    ...
    {
        // 列表
        $data_list = $this->shop_cate->select()->toArray();
        $tree      = new Tree();
    ...
    public function add()
    {
        if(request()->isPost()){
            ...
            // 存储数据
            $ret = $this->shop_cate->save($data_db);
            if ($ret) {
                return $this->return(['code' => 200, 'msg' => '添加成功', 'data' => []]);
            } else {
                return $this->return(['code' => 0, 'msg' => '添加失败:' . $this->shop_cate->getError(), 'data' => []]);
            }
        } else {
            // 获树状列表
    ...
    public function edit($id)
    {
        if(request()->isPut()){
            ...
            // 存储数据
            $ret = $this->shop_cate->update($data_db, ['id' => $id]);
            if ($ret) {
                return $this->return(['code' => 200, 'msg' => '修改成功', 'data' => []]);
            } else {
                return $this->return(['code' => 0, 'msg' => '修改失败:' . $this->shop_cate->getError(), 'data' => []]);
            }
        } else {
            // 获取信息
            $info = $this->shop_cate
    ...
    public function delete($id)
    {
        ...
        if ($ret) {
            ...
            return $this->return(['code' => 0, 'msg' => '删除错误:' . $this->shop_cate->getError(), 'data' => []]);
        }
    }
}
