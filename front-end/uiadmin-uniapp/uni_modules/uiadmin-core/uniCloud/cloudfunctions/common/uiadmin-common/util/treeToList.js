 ...
 */
 ...
 function treeToList(tree) {
   ...
   while (stack.length) {
    ...
    // jry
    // 如果后台没有拼接成api字段，那么这里来拼接。
    if (!node.hasOwnProperty("api")) {
        node.api = node.apiPrefix + "/" + node.menuLayer + node.path;
    }
    // 拼接router-link
    node.routePath = (node?.pathSuffix ? node?.pathSuffix : '')
        + '/pages' + node.path
        + node?.apiParams;
    node.index = node.routePath
        + (node?.queryParams ? '?' +node?.queryParams : '')
