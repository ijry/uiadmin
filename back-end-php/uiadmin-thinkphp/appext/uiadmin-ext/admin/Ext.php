...
class Ext extends BaseAdmin
{
    ...
    public function export()
    {
        ...
        $info = ExtModel::where('id', $id)
            ->find();
        $dir = root_path() . EXT_DIR . '/' . $info['name'];
        if (is_dir($dir)) {
            // 打包下载
    ...
    public function lists()
    {
        ...
        $dirList = \uiadmin\core\util\File::get_dirs(root_path() . '/'. EXT_DIR)['dir'];
        foreach ($dirList as $key => $value) {
            if ($value == '.' || $value == '..') {
                continue;
            }
            if (!in_array($value, $installed_names)) {
                if (is_file(root_path() . EXT_DIR . '/' . $value . '/composer.json')) {
                    $moduleInsall = file_get_contents(root_path() . EXT_DIR . '/' . $value . '/composer.json');
                    $moduleInsall = json_decode($moduleInsall, true);
                    $names = explode('/', $moduleInsall['name']);
                    $module = [
                        'id' => '',
        ...
        if ($type == 'local' || $type == '') {
            ...
            $res = file_get_contents(get_config('uidomain') . '/api/v1/ext/addon/lists?framework=thinkphp', false, stream_context_create($stream_opts));
            if ($res) {
    ...
    public function add()
    {
        if (request()->isPost()) {
            ...
            if (is_dir(root_path() . EXT_DIR . '/' . $dataDb['name'])) {
                return $this->return(['code' => 0, 'msg' => '模块名称已经存在请换一个', 'data' => []]);
            }
            ...
            try {
                ...
                $buildData = [
                    '__file__'   => [
                        ...
                        $moduleNameWithSubdir . '/Service.php'
                    ],
                    '__dir__'    => [
                ...
                $moduleBuilder = new \uiadmin\ext\util\Build(root_path() . EXT_DIR .'/');
                ...
                file_put_contents(root_path() . EXT_DIR . '/' . $moduleNameWithSubdir . '/composer.json', $composerContent);

                // 公共方法
                $commonContent = <<<EOF
                <?php 
EOF;
                file_put_contents(root_path() . EXT_DIR . '/' . $moduleNameWithSubdir . '/function.php', $commonContent);
...
{$dataDb['description']}                
EOF;
                file_put_contents(root_path() . EXT_DIR . '/' . $moduleNameWithSubdir . '/README.md', $readmeContent);
...
EOF;
                file_put_contents(root_path() . EXT_DIR . '/' . $moduleNameWithSubdir . '/Service.php', $serviceContent);
