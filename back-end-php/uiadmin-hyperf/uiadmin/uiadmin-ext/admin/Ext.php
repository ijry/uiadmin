            ...
            try {
                ...
                $buildData = [
                    '__file__'   => [
                        ...
                        $moduleNameWithSubdir . '/ConfigProvider.php'
                    ],
                    '__dir__'    => [
                        $moduleNameWithSubdir . '/controller',
                        $moduleNameWithSubdir . '/admin',
                        $moduleNameWithSubdir . '/console',
                        $moduleNameWithSubdir . '/listener',
                ...
                $composerContentArray = [
                    ...
                    "extra" => [
                        ...
                        "hyperf" => [
                            'config' => $names[0] . "\\".$names[1]."\\ConfigProvider"
                        ],
                        "laravel" => [
                            "providers" =>[
                ...
                file_put_contents(base_path() . EXT_DIR . '/' . $moduleNameWithSubdir . '/composer.json', $composerContent);

                // 公共方法
                $commonContent = <<<EOF
                <?php 
EOF;
                file_put_contents(base_path() . EXT_DIR . '/' . $moduleNameWithSubdir . '/function.php', $commonContent);
...
{$dataDb['description']}                
EOF;
                file_put_contents(base_path() . EXT_DIR . '/' . $moduleNameWithSubdir . '/README.md', $readmeContent);
...
class ConfigProvider
{
    public function __invoke(): array
    {
        return [
            ...
            // 与 commands 类似
            'listeners' => [
                \{$names[0]}\{$names[1]}\listener\EventListener::class, // 注册事件监听器
            ],
...
EOF;
                file_put_contents(base_path() . EXT_DIR . '/' . $moduleNameWithSubdir . '/ConfigProvider.php', $serviceContent);

                // Service
                $eventListenerContent = <<<EOF
<?php
namespace {$names[0]}\\{$names[1]};
...
EOF;
                file_put_contents(base_path() . EXT_DIR . '/' . $moduleNameWithSubdir . '/listener/EventListener.php', $eventListenerContent);
